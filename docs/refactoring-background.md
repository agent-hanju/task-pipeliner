...
> 본 문서는 taxonomy-converter 샘플 구축 과정에서 발견된 안티패턴과, 프레임워크 순수성 점검 결과를 기록한다.
> WBS 수정 계획: `WBS.md`
...
## taxonomy-converter에서 발견된 안티패턴

### A-1. engine.run()에 input_items를 직접 전달

**현상:** 사용자 프로젝트의 실행 스크립트가 `load_items()` 제너레이터를 만들어서 `engine.run(input_items=...)` 으로 전달한다.
...
**현상:** 사용자 프로젝트의 실행 스크립트가 config를 로드한 후 `cfg.pipeline.append(StepConfig(type="write", ...))` 로 writer 스텝을 코드에서 끼워넣는다.
...
### A-3. pipeline_config.yaml이 파이프라인의 일부만 선언
...
### C-1. io.py — JsonlWriter: 도메인 특화 I/O가 프레임워크에 내재
...
### C-2. engine.py — 엔진이 I/O를 수행

| 항목 | 위치 | 문제 |
|------|------|------|
| `input_items` 파라미터 | `run()` 시그니처 | 엔진이 입력 소스를 외부에서 받아 직접 InputProducer에 주입. config-driven이 아님 |
| `InputProducer` 직접 생성 | 내부 | 엔진이 특정 Producer 타입을 하드코딩하여 생성 |
| `output_dir` 파라미터 | `run()` 시그니처 | 엔진이 파일시스템 출력 위치를 알아야 함 |
| `result.write(output_dir)` | 내부 | 엔진이 각 스텝의 결과를 직접 파일에 기록 |
| `stats.write_json()` | 내부 | 엔진이 통계를 직접 파일에 기록 |

**판정:** 엔진의 역할은 "config 읽기 → 스텝 인스턴스화 → 큐 연결 → Producer 실행 → 완료 대기"까지다. 다만 `result.write(output_dir)`와 `stats.write_json()`은 **실행 기록**의 영역이므로, 엔진이 output_dir를 받아서 기록하는 것은 수용 가능하다. BaseResult.write()는 스텝 자신의 실행 결과 요약(count_summary.json 등)이지 파이프라인 출력(kept.jsonl)이 아니기 때문이다.

**핵심 문제는 `input_items` 파라미터와 drain loop (write_kept 호출)이다.**

### C-3. pipeline.py — Pipeline 파사드가 JsonlReader를 강제

| 항목 | 문제 |
|------|------|
| `JsonlReader(inputs)` 직접 생성 | 입력 형식을 JSONL로 강제 |
| `inputs: list[Path]` 파라미터 | Pipeline 파사드가 파일 경로 기반 입력을 전제 |

**판정:** Pipeline은 편의 파사드이므로 특정 입력 형식(JSONL)을 기본 제공하는 것 자체는 문제없다. 그러나 그 구현이 engine.run(input_items=...)에 의존하고 있어서, 엔진의 input_items가 제거되면 파사드도 수정이 필요하다. 수정 방향: Pipeline.run()이 inputs를 받으면 내부적으로 config에 JSONL SOURCE 스텝을 주입하여 engine에 전달. 엔진 자체는 config만 본다.

### C-4. cli.py — filter/batch 커맨드
...
### C-5. io.py — JsonlReader

**판정:** 유틸리티로 존재하는 것은 문제없다. Pipeline 파사드가 이것을 기본 제공하는 것도 편의 기능으로 수용 가능. engine이 강제하지만 않으면 됨.
