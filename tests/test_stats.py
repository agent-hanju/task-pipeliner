...
class TestStepStats:
    ...
    def test_to_dict_keys(self) -> None:
        ...
        s.finish()
        d = s.to_dict()
        assert set(d.keys()) == {"step_name", "passed", "filtered", "errored", "elapsed_seconds"}
...
class TestStatsCollector:
    ...
    def test_increment_multiple_calls(self) -> None:
        c = StatsCollector()
        c.register("step_a")
        c.increment("step_a", "passed", 5)
        c.increment("step_a", "passed", 3)
    ...
    def test_write_json_multiple_steps(self, tmp_path: Path) -> None:
        ...
        c.register("s1")
        c.register("s2")
        c.increment("s1", "passed", 2)
        c.increment("s2", "filtered", 4)
        c.finish("s1")
        c.finish("s2")
        out = tmp_path / "stats.json"
        c.write_json(out)
